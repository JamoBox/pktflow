...
| Rollups | `Sample` on `sni` (first/last SNI seen — flags a session that renegotiates, the analytic payoff for otherwise-opaque HTTPS); `Accumulate` on `handshake_type` (the handshake shapes the session went through); `Accumulate` on `selected_cipher_suite` and on `tls_version_selected` (what the session actually negotiated — "this device negotiated TLS 1.0 with a CBC suite" is a stream-level fact worth surfacing on the stream, not burying in one record). The last two appear only in the ServerHello while `sni` appears only in the ClientHello: no single record carries all four, which is what a **conditional rollup** is (05.4's "absent field on a given packet = no-op"). An earlier draft withheld them because the 09.1 kit's rule 3 then demanded every declared rollup field on *every* canonical sample; rule 3 now checks the union of a case's samples, so a ClientHello + ServerHello pair covers all four |
