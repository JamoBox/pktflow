...
| Byte order | Every multi-byte field in the SMB2 header is **little-endian** ([MS-SMB2] §2.2.1, CIFS/DOS heritage) — the exception in this codebase, where nearly everything else is network byte order. `ProtocolId` is a byte string, and the NBSS length prefix that precedes the message on TCP 445 *is* big-endian, so one message carries both conventions. `StructureSize` MUST be 64, and the plugin enforces it: it is the cheap invariant that turns a byte-order slip into a clean decline instead of plausible-looking garbage (read big-endian, a `TreeConnect` command of 3 becomes 0x0300 and `session_id` — the flow key — comes out byte-reversed) |
